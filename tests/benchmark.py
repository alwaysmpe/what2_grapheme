...
def random_utf_string(n):
    codes: list[int] = list(range(1114111))
...
statements = [
    ...
    ("chunk.length(long_ascii_string)", 500),
...
for statement, n in statements:
    result = timeit.timeit(statement, setup="from __main__ import long_ascii_string; import grapheme; from what2_grapheme import chunk;", number=n) / 100
